      ...
      <div className="flex">
        ...
        <aside className="w-64 border-r border-white/5 h-screen sticky top-0 bg-[#0a0a0a] p-6 hidden lg:flex flex-col text-left">
          ...
          <nav className="space-y-1 flex-1">
            ...
            <button className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-500 hover:text-slate-300 transition-colors text-left">
                <TrendingUp size={18}/>
                <span className="text-sm font-bold tracking-tight">Sharp Trends</span>
            </button>
          </nav>
          ...
          {!isPremium && (
            <div className="p-4 bg-gradient-to-br from-indigo-900/20 to-slate-800/40 rounded-2xl border border-indigo-500/20 mt-auto text-left">
                ...
                className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-all shadow-lg shadow-indigo-900/40"
                >
        ...
        <main className="flex-1 p-4 md:p-10 max-w-7xl mx-auto text-left">
          <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
            <div className="text-left">
              {/* BRANDING FIX: Top Center Header */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-12">
             <div className="bg-[#0f0f0f] p-6 rounded-3xl border border-white/5 flex flex-col items-center justify-center text-center shadow-lg">
             ...
             <div className="bg-[#0f0f0f] p-6 rounded-3xl border border-white/5 flex flex-col items-center justify-center text-center shadow-lg">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Avg. Gap</span>
             ...
             <div className="bg-[#0f0f0f] p-6 rounded-3xl border border-white/5 flex flex-col items-center justify-center text-center shadow-lg">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">Feed Status</span>
                <span className="text-3xl font-black text-green-500 tracking-tighter italic uppercase">Active</span>
            </div>
          </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-white/5">
                  {loading && edges.length === 0 ? (
                      ...
                      <tr><td colSpan="4" className="py-24 text-center text-slate-500 uppercase tracking-widest text-xs font-bold italic">No gaps found. Ensure your scanner is running.</td></tr>
                  ) : (
                    <>
                    {displayedEdges.map((edge, i) => (
                        ...
                        <td className="px-8 py-7">
                            ...
                            <span className="font-black text-xl group-hover:text-indigo-400 transition-colors uppercase italic tracking-tighter leading-none">{edge.player_name || 'NBA Player'}</span>
                            <span className="text-[10px] text-slate-500 uppercase font-black tracking-widest mt-1">{edge.game || 'NBA Game'}</span>
                            </div>
                        </td>
